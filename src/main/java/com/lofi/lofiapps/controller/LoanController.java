...
public class LoanController {
    ...
    public ResponseEntity<ApiResponse<PagedResponse<LoanResponse>>> getLoans(
        ...
        LoanCriteria criteria = LoanCriteria.builder().status(status).branchId(branchId).customerId(customerId).build();
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('SUPER_ADMIN') or hasRole('MARKETING') or hasRole('BRANCH_MANAGER')")
    @RequireIdempotency(ttlHours = 24)
