 ...
 *
 * <p>
 * Per MCP Rules & Workflow Section 7.1 (Marketing Workflow): - Marketing dapat
 * Review Profile -
 * Marketing dapat View Scoring - Marketing TIDAK bisa approve
 *
 * <p>
 * Per Workflow Section 6.2 - Conditional Document Rule (Product-Based)
 */
@Slf4j
@Component
...
public class MarketingReviewLoanUseCase {
   ...
   *
   * <p>
   * Per Notification Workflow Section 4.2: - Trigger: POST /loans/{id}/review -
   * Notify: Customer
   * (FCM), Branch Manager (FCM) - Message: Your loan is under review by marketing
   *
   * @param loanId the loan id to review
   * @return marketing review response with inconsistencies and suggested
   *         questions
   */
  public MarketingLoanReviewResponse execute(java.util.UUID loanId) {
    ...
    boolean hasIncomeProof = docs.stream()
        .anyMatch(
            d -> d.getDocumentType() == DocumentType.PAYSLIP
