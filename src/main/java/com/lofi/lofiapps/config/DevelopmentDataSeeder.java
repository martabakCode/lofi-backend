 ...
 *
 * <p>
 * This seeder ONLY runs when the 'dev' Spring profile is active. It creates
 * comprehensive test
 * data including:
 *
 * <ul>
 ...
 * <li>Loans in various statuses (DRAFT, SUBMITTED, REVIEWED, APPROVED,
 * DISBURSED, etc.)
 ...
 *
 * <p>
 * <strong>WARNING:</strong> This seeder cleans up and recreates transactional
 * data on each run.
 * It should NEVER be used in production.
 *
 * <p>
 * For minimal required data that runs in all environments, see
 * {@link DataInitializer}.
 */
@Configuration
...
public class DevelopmentDataSeeder {
    ...
    /**
     * Seeds development data. Only executes when 'dev' profile is active. Cleans up
     * existing
    ...
    private void initRolesAndPermissions() {
        ...
        Set<Permission> bmPerms = filterPermissions(
    ...
    private void initProducts() {
        ...
        productRepository.findAll().stream()
                .filter(
                        p -> !allowedCodes.contains(p.getProductCode()) && !"KTA-001".equals(p.getProductCode()))
                .forEach(
                        p -> {
    ...
    private void initSlaLoans() {
        ...
        boolean slaExists = loanRepository.findByCustomerId(cust.getId()).stream()
    ...
    private void initPendingSlaLoans() {
        ...
        Loan l1 = loanRepository.findAll().stream()
                .filter(
                        l -> l.getCustomer().getId().equals(cust.getId())
        ...
        Loan l3 = loanRepository.findAll().stream()
                .filter(
                        l -> l.getCustomer().getId().equals(cust.getId())
